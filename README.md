...
| 중복 | 20 | 3줄 이상 구조적 동일 블록 -5/개 |
| 복잡도 | 20 | 중첩 3단계 초과 -5/개, 부정 조건 -2/개 |
| 주석 | 10 | 코드 설명 주석 -2/개, 거짓 주석 -5/개 |

**총점 = 함수/파일 점수(90점) + 아키텍처 점수(10점) = 100점**

- 관점별/총점 하한 0점 (음수 금지)
- God 파일 기준: 300줄 초과 AND export 10개 이상
- 경계 조건: 모두 "초과" 기준 (20줄=통과, 21줄=감점 / 3개 파라미터=통과, 4개=감점 / 3단계 중첩=통과, 4단계=감점)
- 리팩토링 후 반드시 전체 재측정 (Before 감점이 After에도 유효하다고 가정 금지)
...
| 영역 | 신뢰도 | 설명 |
|------|--------|------|
| 함수 줄 수, 중첩 깊이, 매개변수 수 | **높음** | 정량 측정, 도구로 정확 계산 |
| 순환 의존성, import 그래프 | **높음** | 결정 가능한 문제, Grep으로 완전 탐지 |
| 린트/타입체크/테스트 before/after | **높음** | 외부 도구에 위임 |
| SRP 위반 탐지 | **중간** | 줄 수 기반 근사, 짧지만 복수 책임인 함수는 놓침 |
| 중복 탐지 | **중간** | 구조적 동일만 감지, 의미적 중복은 놓침 |
| 추상화 수준 적절성 (KISS vs DRY) | **인간 필요** | 원칙 충돌 — 알고리즘으로 결정 불가능 |
| "이 이름이 충분히 명확한가" (3글자+) | **인간 필요** | 도메인 맥락 의존 |
| YAGNI 위반 여부 | **인간 필요** | 미래 요구사항 지식 필요 |

**이 에이전트는 1차 필터(first-pass filter)로 사용하되, 점수를 절대적 품질 지표로 취급하지 말 것.**
60-80점 구간은 인간 리뷰어 재검토를 권장한다.
