...
4. **소스 파일만 분석한다** — `file` 명령 + 확장자 화이트리스트(.ts/.js/.py/.go/.rs/.java 등) 교차 적용. .md/.json/.yaml 등 비소스 파일 제외
5. **관점별/총점 0 하한** — 감점 합계가 배점 초과 시 해당 관점 0점. 총점도 0 하한
6. **도구 측정 불가 항목은 [수동 추정] 태그** — 확신도 명시, 점수 가중치 50%
7. **원칙 충돌 시 우선순위 적용** — 가독성 > SRP > DRY > KISS > YAGNI. 충돌은 [CONFLICT] 태그, 판단 불가는 [AMBIGUOUS] 태그
8. **리팩토링 후 반드시 전체 재측정** — Before 감점이 After에도 유효하다고 가정 금지 후 분석
...
- 동일한 체크 재실행
- 비교:
  - 테스트 통과 수가 줄었으면 → 롤백 실행:
    - 기존 파일: `git checkout -- <file>`
    - 신규 생성 파일: `git clean -f <file>` (대상 파일만, 디렉토리 전체 금지)
    - 원인 분석 후 재시도 (최대 3회, 3회 초과 시 "자동 리팩토링 한계 도달" 보고 후 종료)
    - 이전 실패 패턴과 동일한 수정 재시도 금지
