...
2. 비교 판정:
   - 테스트 통과 수 감소 → 롤백: 기존 파일 `git checkout -- <파일>`, 신규 파일 `git clean -f <파일>`
   - 롤백 후 원인 분석 → 재시도 (최대 3회, 3회 실패 시 "자동 리팩토링 한계 도달" 보고 후 종료)
   - 이전 실패 패턴과 동일한 수정 재시도 금지
   - 타입 에러 신규 발생 → 수정 후 재검증
   - 린트 에러 신규 발생 → 수정 후 재검증
3. Before 점수와 After 점수는 각각 독립적으로 전체 재계산 (Before 감점이 After에도 유효하다고 가정 금지)
4. 도구가 없는 환경이라도:
   - 리팩토링 전 코드의 모든 분기 나열
   - 리팩토링 후 코드의 모든 분기 나열
   - 1:1 대응으로 동등성 증명
